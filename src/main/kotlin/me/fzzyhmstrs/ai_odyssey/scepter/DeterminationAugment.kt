...
class DeterminationAugment(tier: Int, maxLvl: Int, vararg slot: EquipmentSlot): MinorSupportAugment(tier, maxLvl, *slot),
    HealerAugment {

    override val baseEffect: AugmentEffect
        get() = super.baseEffect.withAmplifier(0).withDuration(700,100)
        ...
        if(target != null) {
            if (target is PassiveEntity || target is GolemEntity || target is PlayerEntity) {
                EffectQueue.addStatusToQueue(target as LivingEntity,
                    RegisterStatus.DETERMINATION, effects.duration(level), effects.amplifier(level))
    ...
    override fun augmentStat(imbueLevel: Int): AugmentDatapoint {
        return AugmentDatapoint(SpellType.GRACE,1800,15,3,imbueLevel, LoreTier.LOW_TIER, Items.SHIELD)
    }
}